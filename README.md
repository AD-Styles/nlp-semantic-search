...
## 💡 회고록 (Retrospective)
  ...
  <br>&emsp;위와 같은 한계점을 토대로 단순히 가장 유사한 문서를 찾아주는 것을 넘어, 검색된 문맥(Context)을 대형 언어 모델(LLM)에 주입하여 사용자 질문에 대한 맞춤형 답변을 직접 생성해 내는 RAG(검색 증강 생성) 시스템으로 파이프라인을 고도화해 보고 싶습니다.
