...
## 💡 회고록 (Retrospective)
  ...
  &emsp;하지만 실제 구어체 대화 데이터를 다루어 보니, 문장의 전체적인 뉘앙스보다 특정 명사(예: '날씨', '회사')에 벡터의 방향이 과도하게 끌려가는 '키워드 편향' 현상을 목격하며 단순 문장 임베딩의 한계점을 발견했습니다. 또한 입력된 질문과 데이터베이스 내의 모든 문장을 일일이 대조하여 코사인 유사도를 연산하는 현재의 선형 탐색(Linear Search) 방식은, 데이터 크기에 비례해 연산량이 증가하여 대규모 환경에서는 심각한 속도 저하와 병목을 일으킨다는 구조적 한계에 직면했습니다.
&emsp;위와 같은 한계점을 토대로 단순히 가장 유사한 문서를 찾아주는 것을 넘어, 검색된 문맥(Context)을 대형 언어 모델(LLM)에 주입하여 사용자 질문에 대한 맞춤형 답변을 직접 생성해 내는 RAG(검색 증강 생성) 시스템으로 파이프라인을 고도화해 보고 싶습니다.
